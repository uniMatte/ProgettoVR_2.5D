...
namespace WeArt.Components
{
    ...
    public class WeArtDeviceTrackingObject : MonoBehaviour
    {
        ...
        private void UpdateHands()
        {
            ...
            if (_showGhostHand)
            {
                ...
                {
                    if (_ghostHandRenderer.sharedMaterial != _handController.GetGhostHandTransparentMaterial())
                        _ghostHandRenderer.sharedMaterial = _handController.GetGhostHandTransparentMaterial();
                }
                else
                {
                    if (_ghostHandRenderer.sharedMaterial != _handController.GetGhostHandInvisibleMaterial())
        ...
        private void UpdateRigidbody()
        {
            ...
            if (_isAnchored)
            {
                if (_handController.GetGraspingSystem().GetGraspedObject().GetComponent<ConfigurableJoint>() != null)
                {
                    ...
                    _handController.GetGraspingSystem().GetGraspedObject().GetComponent<Rigidbody>().linearVelocity =
    ...
    #if UNITY_6000_0_OR_NEWER
            _rigidBody.linearVelocity = (TrackingSource.TransformPoint(_positionOffset) - transform.position) / Time.fixedDeltaTime * (_safeReleaseSeconds > 0 ? _safeReleaseVelocityPower : _handFollowSpeed);
    #else
            ...
            {
            #if UNITY_6000_0_OR_NEWER
                _rigidBody.linearVelocity = _handFollowPowerDuringGrab * _rigidBody.linearVelocity;

                if (_rigidBody.linearVelocity.magnitude > 3)
                {
                    _rigidBody.linearVelocity = _rigidBody.linearVelocity.normalized * 3;
                }
            #else
