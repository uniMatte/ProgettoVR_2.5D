...
public class WeArtHandGraspingSystem : MonoBehaviour
{
    ...
    /// <summary>
    /// Defines the grasp and release threshold for proximity grasping mode
    /// </summary>
    [Range(0f, 1f)][SerializeField]
    internal float _fingerClosureGraspThreshold, _fingerClosureReleaseThreshold;
    ...
    /*/// <summary>
    /// Defines if the hand is visible during grasp
    /// </summary>
    [SerializeField]
    internal bool _hideDuringGrasp = false;*/

    /// <summary>
    /// Defines if the hand is physical during grasp
    /// </summary>
    [SerializeField]
    internal bool _notPhysicalDuringGrasp = false;
    ...
    /// <summary>
    /// Defines the closes touchable object with proximity grasping active. Will be null if there are no objects in proximity
    /// </summary>
    private WeArtTouchableObject _proximityGraspClosestTouchable;

    private WeArtEasyGraspPose _snapGraspPose;

    const float GraspingThreshold = 0.05f;

    private Coroutine _enableDisableCollidersCoroutine;

    private List<WeArtTouchableObject> _touchablesRemoveList = new List<WeArtTouchableObject>();
    ...
    /*/// <summary>
    /// Check the hand visibility during grasp
    /// </summary>
    public bool HideDuringGrasp
    {
        get => _hideDuringGrasp;
        set => _hideDuringGrasp = value;
    }*/

    /// <summary>
    /// Check the if the hand is physical during grasp
    /// </summary>
    public bool NotPhysicalDuringGrasp
    {
        get => _notPhysicalDuringGrasp;
        set => _notPhysicalDuringGrasp = value;
    }
    ...
    private void Awake()
    {
        ...
        WeArtHandController[] components = GameObject.FindObjectsByType<WeArtHandController>(FindObjectsSortMode.None);
#else
    ...
    public void UpdateGraspingSystem()
    {
        ...
        // Proximity
        if (WeArtController.Instance.UseExternalGraspSystem) HandleProximityGrasping();
        ...
        if(!(_graspingState == GraspingState.Grabbed && _graspedObject.GraspingType == GraspingType.Proximity)) 
            CheckAllGraspingEffects();

        CheckFingerBlocking();

        if(!WeArtController.Instance.UseExternalGraspSystem) CheckGraspingConditions();

        HandleFingerEffects();

        CheckObjectsCollisionOnRelease();
    }

    void CheckObjectsCollisionOnRelease()
    {
       // if(Input.GetKeyDown(KeyCode.Alpha1))
        if (_graspingState == GraspingState.Released)
        {
            RemoveNotUsedTouchables(_thumbThimbleHaptic, _thumbThimbleTracking);
            RemoveNotUsedTouchables(_indexThimbleHaptic, _indexThimbleTracking);
            RemoveNotUsedTouchables(_middleThimbleHaptic, _middleThimbleTracking);
            RemoveNotUsedTouchables(_annularThimbleHaptic, _annularThimbleTracking);
            RemoveNotUsedTouchables(_pinkyThimbleHaptic, _pinkyThimbleTracking);
            RemoveNotUsedTouchables(_palmThimbleHaptic);
        }
    }

    void RemoveNotUsedTouchables(WeArtHapticObject haptic, WeArtThimbleTrackingObject thimbleTracking = null)
    {
        _touchablesRemoveList.Clear();
        foreach (var touchable in haptic.TouchedObjects)
        {
            bool foundCollision = false;
            foreach (var item in touchable.GetTouchableColliders())
            {
                if (Physics.ComputePenetration(item, item.transform.position, item.transform.rotation, haptic.GetComponent<Collider>(), haptic.transform.position, haptic.transform.rotation, out Vector3 dir, out float dis))
                {
                    foundCollision = true;
                    break;
                }
            ...
            if (!foundCollision)
            {
                //Debug.LogError("Found not used");
                _touchablesRemoveList.Add(touchable);
            }
        }

        for (int i = _touchablesRemoveList.Count - 1; i >= 0; i--)
        {
            haptic.RemoveTouchedObject(_touchablesRemoveList[i]);
        }

        if (haptic.TouchedObjects.Count <= 0)
        {
            if (haptic.ActiveEffect != null)
                haptic.RemoveEffect();

            if(thimbleTracking != null)
            {
    ...
    /// <summary>
    /// Enable or Disable hand colliders
    /// </summary>
    /// <param name="value"></param>
    public void EnableOrDisableHandColliders(bool value) {
        _enableDisableCollidersCoroutine = StartCoroutine(EnableOrDisableHandColliderCoroutine(value));
    }

    IEnumerator EnableOrDisableHandColliderCoroutine(bool value)
    {
        if(value) yield return new WaitForSeconds(1f);

        List<Collider> colliders = new List<Collider>(GetComponentsInChildren<Collider>(true).Where(x => !x.isTrigger));
        ...
        if (value) _handController._handsSystem.HandlePhysicsType();
    }
        ...
        else
        {
            foreach (WeArtTouchableObject contactItem in contactTouchables)
            {
                foreach (WeArtTouchableObject graspCheckItem in graspCheckTouchables)
                {
                    if (contactItem == graspCheckItem)
                    {
                        if(!graspCheckItem.GetComponent<WeArtVolumeEffects>()) thimbleTracking.IsBlocked = true;
    ...
    private void HandleProximityGrasping() {
        if (_graspingState == GraspingState.Released)
        {
            if (IsTryingToProximityGrasp()) {
                GraspTouchableObject(_proximityGraspClosestTouchable);
            }
        }
        else {
            if (IsTryingToProximityRelease())
            {
    ...
    private bool IsTryingToProximityGrasp() {

        int fingersInRange = 0;
        int fingersInRangeAndClosed = 0;

        if (_thumbContactTouchables.Count > 0) {
            fingersInRange++;
            if (_thumbThimbleTracking.Closure.Value >= _fingerClosureGraspThreshold) 
                fingersInRangeAndClosed++;
        }

        if (_indexContactTouchables.Count > 0) {
            fingersInRange++;
            if (_indexThimbleTracking.Closure.Value >= _fingerClosureGraspThreshold) 
                fingersInRangeAndClosed++;
        }

        if (_middleContactTouchables.Count > 0) {
            fingersInRange++;
            if (_middleThimbleTracking.Closure.Value >= _fingerClosureGraspThreshold) 
                fingersInRangeAndClosed++;
        }

        if (_annularContactTouchables.Count > 0) {
            fingersInRange++;
            if (_annularThimbleTracking.Closure.Value >= _fingerClosureGraspThreshold) 
                fingersInRangeAndClosed++;
        }

        if (_pinkyContactTouchables.Count > 0) {
            fingersInRange++;
            if (_pinkyThimbleTracking.Closure.Value >= _fingerClosureGraspThreshold) 
                fingersInRangeAndClosed++;
        }

        if (fingersInRange > 1)
        {
            _proximityGraspClosestTouchable = FindProximityTouchableObject(new List<List<WeArtTouchableObject>>
                { _thumbContactTouchables, _indexContactTouchables, _middleContactTouchables, _annularContactTouchables, _pinkyContactTouchables });

            if (_proximityGraspClosestTouchable != null && !_proximityGraspClosestTouchable.IsGraspable) return false;

            if (_proximityGraspClosestTouchable != null && !_proximityGraspClosestTouchable.IsReadyToBeGrasped && _proximityGraspClosestTouchable.GetGraspingState() != GraspingState.Grabbed)
                _proximityGraspClosestTouchable.GrabReady(_grasper);
        }
        else {
            if (_proximityGraspClosestTouchable != null && !_proximityGraspClosestTouchable.IsGraspable) return false;

            if (_proximityGraspClosestTouchable != null && _proximityGraspClosestTouchable.IsReadyToBeGrasped && _proximityGraspClosestTouchable.GetGraspingState() != GraspingState.Grabbed)
                _proximityGraspClosestTouchable.GrabNotReady(_grasper);
        }

        if (fingersInRangeAndClosed > 1)
        {
            // Check intentional grasping
            if (_thumbThimbleTracking.Closure.Value - _lastThumbClosure > GraspingThreshold ||
                _indexThimbleTracking.Closure.Value - _lastIndexClosure > GraspingThreshold ||
                _middleThimbleTracking.Closure.Value - _lastMiddleClosure > GraspingThreshold ||
                _annularThimbleTracking.Closure.Value - _lastAnnularClosure > GraspingThreshold ||
                _pinkyThimbleTracking.Closure.Value - _lastPinkyClosure > GraspingThreshold)
            {
                _proximityGraspClosestTouchable = FindProximityTouchableObject(new List<List<WeArtTouchableObject>>
                { _thumbContactTouchables, _indexContactTouchables, _middleContactTouchables, _annularContactTouchables, _pinkyContactTouchables });
                ...
                if (_proximityGraspClosestTouchable != null && _proximityGraspClosestTouchable.IsReadyToBeGrasped)
                    _proximityGraspClosestTouchable.GrabNotReady(_grasper);

                return _proximityGraspClosestTouchable != null;
            }
        }
        return false;
    }
    ...
    /// <summary>
    /// Get the touchable object that is present in the most finger contact lists
    /// </summary>
    WeArtTouchableObject FindProximityTouchableObject(List<List<WeArtTouchableObject>> lists)
    {
        Dictionary<WeArtTouchableObject, int> touchablesDictionary = new Dictionary<WeArtTouchableObject, int>();

        foreach (var list in lists)
        {
            if (list == null) continue;

            foreach (var item in new HashSet<WeArtTouchableObject>(list))
            {
                if (!touchablesDictionary.ContainsKey(item))
                    touchablesDictionary[item] = 0;

                touchablesDictionary[item]++;
            }
        }

        if (touchablesDictionary.Count == 0)
            return default;

        return touchablesDictionary.OrderByDescending(x => x.Value).First().Key;
    }

    private bool IsTryingToProximityRelease() {
        int fingersNotInRange = 0; 

        if (_thumbThimbleTracking.Closure.Value <= _fingerClosureReleaseThreshold)
            fingersNotInRange += 1;

        if (_indexThimbleTracking.Closure.Value <= _fingerClosureReleaseThreshold)
            fingersNotInRange += 1;

        if (_middleThimbleTracking.Closure.Value <= _fingerClosureReleaseThreshold)
            fingersNotInRange += 1;

        if (_annularThimbleTracking.Closure.Value <= _fingerClosureReleaseThreshold)
            fingersNotInRange += 1;

        if (_pinkyThimbleTracking.Closure.Value <= _fingerClosureReleaseThreshold)
            fingersNotInRange += 1;

        if (fingersNotInRange >= 4)
            return true;
    ...
    private bool IsTryingToSnapGrasp()
    {
        ...
        if (_thumbThimbleTracking.Closure.Value > GetSnapFingerThreshold(_thumbThimbleTracking))
            fingersInRange += 1;

        if (_indexThimbleTracking.Closure.Value > GetSnapFingerThreshold(_indexThimbleTracking))
            fingersInRange += 1;

        if (_middleThimbleTracking.Closure.Value > GetSnapFingerThreshold(_middleThimbleTracking))
            fingersInRange += 1;

        if (_annularThimbleTracking.Closure.Value > GetSnapFingerThreshold(_annularThimbleTracking))
            fingersInRange += 1;

        if (_pinkyThimbleTracking.Closure.Value > GetSnapFingerThreshold(_pinkyThimbleTracking))
            fingersInRange += 1;
    ...
    private float GetSnapFingerThreshold(WeArtThimbleTrackingObject thimbleTraking)
    {
        ...
        float snapGraspSensivity = 0.8f; //Update from 0.5f to 0.8f for TDPro
    ...
    {
        int fingersStillInRange = 0;

        if (_thumbThimbleTracking.Closure.Value > GetSnapFingerThreshold(_thumbThimbleTracking))
            fingersStillInRange += 1;
    ...
    private void GraspTouchableObject(WeArtTouchableObject touchable)
    {
        if (_otherGraspingSystem != null)
        {
            ...
            {
                if (_otherGraspingSystem.GetGraspedObject() == touchable && touchable.GraspingType != GraspingType.Proximity)
                {
        ...
        if (_enableDisableCollidersCoroutine != null) StopCoroutine(_enableDisableCollidersCoroutine); 

        _graspingState = GraspingState.Grabbed;
        _graspedObject = touchable;
        OnGraspingEvent?.Invoke(_handSide, _graspedObject.gameObject);

        if (_graspedObject.GraspingType != GraspingType.Proximity)
        {
            _graspedObject.GraspingHandController = _handController;
        }
        else { 
            if(_graspedObject.GraspingHandController == null) 
            {
                _graspedObject.GraspingHandController = _handController;
            }else
            {
                _graspedObject.OtherGraspingHandController = _handController;
            }
        }
    ...
    public void ReleaseTouchableObject()
    {
        ...
        if (_graspedObject != null)
        {
            ...
            _graspedObject.Release(_grasper);

            if (_graspedObject.GraspingType != GraspingType.Proximity)
            {
                _graspedObject.GraspingHandController = null;
            }else {
                if (_graspedObject.GraspingHandController == _handController)
                {
                    _graspedObject.GraspingHandController = null;
                }
                else if (_graspedObject.OtherGraspingHandController == _handController)
                {
                    _graspedObject.OtherGraspingHandController = null;
                }
            }
        }
        ...
        if (_graspedObject.GraspingType == GraspingType.Proximity && NotPhysicalDuringGrasp) {
            _thumbContactTouchables.Clear();
            _indexContactTouchables.Clear();
            _middleContactTouchables.Clear();
            _annularContactTouchables.Clear();
            _pinkyContactTouchables.Clear();
        }
    ...
    void FingerTriggerEnterHandle(Collider other, WeArtHapticObject hapticObject, WeArtThimbleTrackingObject thimbleTrackingObject, List<WeArtTouchableObject> contactTouchables, List<WeArtTouchableObject> graspCheckTouchables, int fingerNumber)
    {
        if (TryGetTouchableObjectFromCollider(other, out var touchable))
        {
            ...
            if (other.isTrigger && touchable.GraspingType == GraspingType.Proximity && other.GetComponent<WeArtChildCollider>() && !other.GetComponent<WeArtVolumeEffects>()) return;
