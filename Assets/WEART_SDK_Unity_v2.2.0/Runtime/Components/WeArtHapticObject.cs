...
namespace WeArt.Components
{
    ...
    public class WeArtHapticObject : MonoBehaviour
    {
        ...
        public Force CalculatePhysicalForce(WeArtTouchableObject touchable)
        {
            ...
            if (_distanceForceTarget != null && touchable._disableDynamicForce == false)
            {
                ...
                float max_distance_covered = GetInitialMaxDistanceForActPoint(_actuationPoints, _graspedTouchableObjectForce, _isGrasping, WeArtController.Instance.DeviceGeneration);
            ...
            if(WeArtController.Instance.DeviceGeneration == DeviceGeneration.TD_Pro && _isGrasping == true)
            {
                pressure_force = _graspedTouchableObjectForce;
            }
        ...
        public void HandleOnTriggerStay(Collider collider)
        {
            if (TryGetTouchableObjectFromCollider(collider, out var touchable))
            {
                if (collider.isTrigger && touchable.GraspingType == GraspingType.Proximity && collider.GetComponent<WeArtChildCollider>() && !collider.GetComponent<WeArtVolumeEffects>()) return;
