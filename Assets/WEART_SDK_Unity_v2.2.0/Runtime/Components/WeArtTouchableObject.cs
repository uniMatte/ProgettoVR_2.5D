...
namespace WeArt.Components
{
    ...
    public class WeArtTouchableObject : MonoBehaviour
    {
        ...
        /// <summary>
        /// Defines if the haptic objects should have dynamic force while touching this touchable object
        /// </summary>
        [SerializeField] internal bool _disableDynamicForce = false;
        ...
        /// <summary>
        /// The other hand controller that is curently grasping the object
        /// </summary>
        private WeArtHandController _otherGraspingHandController;
        ...
        /// <summary>
        /// Defines if the touchable object is ready to be grasped
        /// </summary>
        private bool _isReadyToBeGrasped = false;
        ...
        /// <summary>
        /// Notifies grasping status changes
        /// </summary>
        [System.Serializable]
        public class GraspingEvent : UnityEvent<GameObject, HandSide> { }
        [SerializeField] internal GraspingEvent OnGraspReady;
        [SerializeField] internal GraspingEvent OnGraspNotReady;
        [SerializeField] internal GraspingEvent OnGrasp;
        [SerializeField] internal GraspingEvent OnRelease;
        ...
        /// <summary>
        /// Gets and Sets the flag the disables the dynamic computation of force for this touchable object
        /// </summary>
        public bool DisableDynamicForce
        {
            get { return _disableDynamicForce; }
            set { _disableDynamicForce = value; }
        }
        ...
        /// Gets and sets the hand controller that is grasping the object
        /// </summary>
        public WeArtHandController OtherGraspingHandController
        {
            get { return _otherGraspingHandController; }
            set { _otherGraspingHandController = value; }
        }
        ...
        /// <summary>
        /// Gets a value indicating whether IsReadyToBeGrasped.
        /// </summary>
        public bool IsReadyToBeGrasped
        {
            get => _isReadyToBeGrasped;
        }
        ...
        private void Update()
        {
            if (_graspingState == GraspingState.Grabbed && _graspingType == GraspingType.Proximity) {
                SetFakeGraspHaptics();
            }
        }
        ...
        {
            if (_graspingState == GraspingState.Grabbed && _graspingType == GraspingType.Proximity) return;
        ...
        public void SetFakeGraspHaptics()
        {
            WeArtTouchEffect effect = new WeArtTouchEffect();
            Force force = Force.Default;
            force.Active = Stiffness.Active;
            force.Value = Stiffness.Value;
            effect.Set(Temperature, force, Texture, null);
            if (_graspingHandController != null) SetFakeHapticEffectOnController(_graspingHandController, effect);
            if (_otherGraspingHandController != null) SetFakeHapticEffectOnController(_otherGraspingHandController, effect);
        }

        void SetFakeHapticEffectOnController(WeArtHandController controller, WeArtTouchEffect effect)
        {
            SetFakeHapticEffectToHapticObject(controller._thumbThimbleHaptic, effect);
            SetFakeHapticEffectToHapticObject(controller._indexThimbleHaptic, effect);
            SetFakeHapticEffectToHapticObject(controller._middleThimbleHaptic, effect);
            if (WeArtController.Instance.DeviceGeneration == DeviceGeneration.TD_Pro)
            {
                SetFakeHapticEffectToHapticObject(controller._annularThimbleHaptic, effect);
                SetFakeHapticEffectToHapticObject(controller._pinkyThimbleHaptic, effect);
                SetFakeHapticEffectToHapticObject(controller._palmThimbleHaptic, effect);
            }
        }

        void SetFakeHapticEffectToHapticObject(WeArtHapticObject haptic, WeArtTouchEffect effect) {
            if (haptic.ActiveEffect == effect) return;
        ...
        public void SaveCurrentRigidbodyParams()
        {
            ...
            _originalAngularDrag = _rigibody.angularDamping;
            _originalDrag = _rigibody.linearDamping;
#else
...
#if UNITY_6000_0_OR_NEWER
                _rigibody.angularDamping = _originalAngularDrag;
                _rigibody.linearDamping = _originalDrag;
#else
        ...
        /// <summary>
        /// The grab ready method called by the hand controller
        /// </summary>
        /// <param name="grasper">The grasper<see cref="GameObject"/>.</param>
        public void GrabReady(GameObject grasper) {
            if (grasper.GetComponentInParent<WeArtHandController>()) {
                _isReadyToBeGrasped = true;
                OnGraspReady?.Invoke(gameObject, grasper.GetComponentInParent<WeArtHandController>()._handSide);
                WeArtGraspProvider.InvokeGraspReady(gameObject, grasper.GetComponentInParent<WeArtHandController>()._handSide);
            }
        }
        ...
        /// </summary>
        /// <param name="grasper">The grasper<see cref="GameObject"/>.</param>
        public void GrabNotReady(GameObject grasper) {
            if (grasper.GetComponentInParent<WeArtHandController>()) {
                _isReadyToBeGrasped = false;
                OnGraspNotReady?.Invoke(gameObject, grasper.GetComponentInParent<WeArtHandController>()._handSide);
                WeArtGraspProvider.InvokeGraspNotReady(gameObject, grasper.GetComponentInParent<WeArtHandController>()._handSide);
            }
        }
        ...
        {
            if (_graspingType == GraspingType.Proximity) {
                WeArtHandController _controller;
                if (_graspingHandController != null && _graspingHandController.gameObject == grasper.transform.parent.gameObject)
                {
                    _controller = _graspingHandController;
                }
                else
                {
                    _controller = _otherGraspingHandController;
                }

                _graspingState = GraspingState.Grabbed;

                _controller.GetGraspingSystem().EnableOrDisableHandColliders(!_controller.GetGraspingSystem().NotPhysicalDuringGrasp);
                _controller.HandleHandVisibility();

                SetFakeGraspHaptics();

                OnGraspingStateChanged?.Invoke(_graspingState);
                InvokeEvents(true, _controller);
                return;
            }
            ...
            if (_anchoredObject != null)
            {
                ...
                _graspingState = GraspingState.Grabbed;
                OnGraspingStateChanged?.Invoke(_graspingState);
                InvokeEvents(true, _graspingHandController);
                return;
            }
            ...
            _graspingHandController.GetGraspingSystem().EnableOrDisableHandColliders(!_graspingHandController.GetGraspingSystem().NotPhysicalDuringGrasp);
            _graspingHandController.HandleHandVisibility();
        ...
        /// </summary>
        public void Release(GameObject grasper)
        {
            if (_graspingType == GraspingType.Proximity)
            {
                WeArtHandController _controller;
                ...
                _controller.GetComponent<WeArtDeviceTrackingObject>().StartSafeRelease();

                _controller.GetGraspingSystem().EnableOrDisableHandColliders(true);
                _controller.HandleHandVisibility();

                TouchedHapticsEffects.Clear();

                OnGraspingStateChanged?.Invoke(_graspingState);
                InvokeEvents(false, _controller);
                return;
            }
            ...
            if (_anchoredObject != null)
            {
                ...
                _graspingState = GraspingState.Released;
                OnGraspingStateChanged?.Invoke(_graspingState);
                InvokeEvents(false, _graspingHandController);
                return;
            }
            ...
            _graspingHandController.GetGraspingSystem().EnableOrDisableHandColliders(true);
            _graspingHandController.HandleHandVisibility();
        ...
        /// <summary>
        /// Invoke the grasp/release events
        /// </summary>
        /// <param name="isGrabbed"></param>
        /// <param name="controller"></param>
        void InvokeEvents(bool isGrabbed, WeArtHandController controller) {
            if (isGrabbed)
            {
                OnGrasp?.Invoke(gameObject, controller._handSide);
                WeArtGraspProvider.InvokeGrasp(gameObject, controller._handSide);
            }
            else
            {
                OnRelease?.Invoke(gameObject, controller._handSide);
                WeArtGraspProvider.InvokeRelease(gameObject, controller._handSide);
            }
        }
