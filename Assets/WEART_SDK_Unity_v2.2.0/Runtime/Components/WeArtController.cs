...
namespace WeArt.Components
{
    ...
    public class WeArtController : MonoBehaviour
    {
        ...
        [SerializeField]
        internal WeArtDeviceObject _deviceFirst;
        [SerializeField]
        internal WeArtDeviceObject _deviceSecond;

        [SerializeField]
        internal LogLevel _projectLogLevel; 

        [SerializeField]
        internal TrackingPerformance _trackingPerformance = TrackingPerformance.HIGH;
        ...
        [SerializeField] 
        internal bool _useExternalGraspSystem = false;

        [SerializeField] 
        internal bool _allowGestures;
        ...
        [SerializeField]
        internal bool _debugMessagesOnConsole = false;

        [SerializeField]
        internal bool _debugMessagesOnFile = false;
        ...
        public static WeArtController Instance
        {
         ...
         _instance = FindFirstObjectByType<WeArtController>();
#else
        ...
        /// Public property about Device Generation G1 and G2 capabilities
        /// </summary>
        public bool UseExternalGraspSystem
        {
            get { return _useExternalGraspSystem; }
            set { _useExternalGraspSystem = value; }
        }
...
#if UNITY_6000_0_OR_NEWER
        _calibrationManager = FindFirstObjectByType<CalibrationManager>();
#else
        _calibrationManager = FindObjectOfType<CalibrationManager>();
#endif
            _weArtTrackingCalibration = GetComponent<WeArtTrackingCalibration>();
        ...
        public void StartCalibration()
        {
            Client.StartCalibration();
#if UNITY_ANDROID && !UNITY_EDITOR
            StartCalibrationBLETdPro();           
#endif
        }

        public void StartDeviceAndCalibration()
        {
            Client.StartCalibration();
#if UNITY_ANDROID && !UNITY_EDITOR
            StartDeviceAndCalibrationBLETdPro();
#endif
        }
        ...
        {
            //Client.StartSensorsCalibration();
#if UNITY_ANDROID && !UNITY_EDITOR
           StartSensorsCalibrationBLETdPro();
#endif
        }
        ...
        /// <summary>
        /// Send message to force calibration status both devices only in Android Standalone platform
        /// </summary>
        private void StartDeviceAndCalibrationBLETdPro()
        {
            _deviceFirst.EnableRunningON();
            _deviceSecond.EnableRunningON();
        }

        /// <summary>
        /// Send message to force calibration status both devices only in Android Standalone platform
        /// </summary>
        private void StartCalibrationBLETdPro()
        {
           _deviceFirst.EnableRunningAlgoCalibration();
           _deviceSecond.EnableRunningAlgoCalibration();
        }

        /// <summary>
        /// Send message to stop devices from RUN only in Android Standalone platform
        /// </summary>
        private void StopRunBLETdPro()
        {
            _deviceFirst.StopRun();
            _deviceSecond.StopRun();
        }

        /// <summary>
        /// Send message to force calibration status both devices only in Android Standalone platform
        /// </summary>
        private void StartSensorsCalibrationBLETdPro()
        {
            _deviceFirst.StartSensorsCalibration();
            _deviceSecond.StartSensorsCalibration();
        }
        ...
        public void ConnectDevice(BleDevice device, HandSide handSide)
        {
            if (_deviceFirst.BleDevice == null)
            {
                ...
                _deviceFirst.ConnectDevice(device, handSide);
                return;
            }
            _deviceSecond.ConnectDevice(device, handSide);
        ...
        private IEnumerator UpdateConnectedHandsDataInCalibrationManagerCor()
        {
            ...
            if (_bluetoothManager.GetConnectedDeviceNumber() < 1)
            {
                _calibrationManager.SetHandDominationData(0);
                yield break;
            }
        ...
        {
#if UNITY_6000_0_OR_NEWER
            WeArtHandController[] controllers = FindObjectsByType<WeArtHandController>(FindObjectsSortMode.None);
#else
            ...
            if (!_weArtHandControllerRight && !_weArtHandControllerLeft)
            {
#if UNITY_6000_0_OR_NEWER
                _weArtHapticObjects = FindObjectsByType<WeArtHapticObject>(FindObjectsSortMode.None);
#else
                ...
                default:
                    {
                        ...
                        {
                            var device = _bluetoothManager.GetDeviceFromMac(macAddress);
        ...
        private void DeviceDisconnectionHandler(BleDevice device)
        {
            ...
            {
                ResetCalibration();
                if (_bluetoothManager.GetConnectedDeviceNumber() > 0)
                {
        ...
        private void DeviceReadyHandler(BleDevice device)
        {
            ...
            StartCoroutine(ConnectConfirm(device));
        }
        ...
        private void DeviceRemovedHandler(BleDevice device)
        {
            ...
            if (device == null)
            {
                WeArtLog.LogFile(LogLevel.DEBUG, PackTag.EVENTS, $"DeviceRemovedHandler called with NULL as device");
                return;
            }
        ...
        private IEnumerator ConnectConfirm(BleDevice device)
        {
            ...
            if (_deviceGeneration == DeviceGeneration.TD)
                ...
                _bluetoothManager.SendData(device, DeviceCommands.CONNECT_CONFIRM_TDPRO);
        ...
        /// <summary>
        /// Callback receiving hand side during connection from WeartApp
        /// </summary>
        /// <param name="bleDevice"></param>
        /// <param name="handSide"></param>
        /// <param name="deviceID"></param>
        internal void OnConnectionHandSideNotity(BleDevice bleDevice, WeartSdkPRO.HAND_SIDE handSide, int deviceID)
        {
            if ((int)_deviceFirst._deviceID == deviceID)
            {
                _deviceFirst._handSide = handSide == WeartSdkPRO.HAND_SIDE.RIGHT ? HandSide.Right : HandSide.Left;
                _bleConnectionPanel.ModifyHandSideOnConnectionPanel_TDPro(bleDevice, handSide, deviceID);
            }
            if ((int)_deviceSecond._deviceID == deviceID)
            {
                _deviceSecond._handSide = handSide == WeartSdkPRO.HAND_SIDE.RIGHT ? HandSide.Right : HandSide.Left;
                _bleConnectionPanel.ModifyHandSideOnConnectionPanel_TDPro(bleDevice, handSide, deviceID);
            }
        }
