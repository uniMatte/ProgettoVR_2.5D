...
{
    private const int SensorsCalibDateDays = 7;
    ...
    [SerializeField] internal GameObject SensorsCalibDateImage;
    [SerializeField] internal TextMeshProUGUI CalibrationText;
    [SerializeField] internal Transform SignalStrenghtDisplay;

    [SerializeField] internal Transform BatteryPercentagePanel;
    [SerializeField] internal Transform NoBatteryPanel;

    [Min(0)]   
    [SerializeField] internal float CalibrationFadeOutTimeSeconds = 2;
    ...
    private Image _firstSignalLine;
    private Image _secondSignalLine;
    private Image _thirdSignalLine;
    private WeArtWatchPanel _watchPanel;
    private GameObject _signalPanelObj;
    private GameObject _calibrationPanelObj;
    ...
    private void Start()
    {
        _calibrationPanel = CalibrationText.transform.parent;
        _signalPanelObj = SignalStrenghtDisplay.gameObject;
        _calibrationPanelObj = _calibrationPanel.gameObject;
        _firstSignalLine = SignalStrenghtDisplay.transform.GetChild(0).GetComponent<Image>();
        _secondSignalLine = SignalStrenghtDisplay.transform.GetChild(1).GetComponent<Image>();
        _thirdSignalLine = SignalStrenghtDisplay.transform.GetChild(2).GetComponent<Image>();


#if UNITY_6000_0_OR_NEWER
                WeArtWatchPanel[] weArtWatchPanels = GameObject.FindObjectsByType<WeArtWatchPanel>(FindObjectsInactive.Include,FindObjectsSortMode.None);
#else
                WeArtWatchPanel[] weArtWatchPanels = GameObject.FindObjectsOfType<WeArtWatchPanel>(true);
#endif

        foreach (var item in weArtWatchPanels)
        {
            if(item.GetHandSide() == _handSide)
            {
                _watchPanel = item;
            }
        }
    ...
    private void UpdatePanels()
    {
        ...
        {
            UpdateInfoPanel(_currentTDStatus.MacAddress, _currentTDStatus.BatteryLevel, _currentTDStatus.Charging, 0);
        ...
        {
            UpdateInfoPanel(_currentTDProStatus.MacAddress, _currentTDProStatus.Master.BatteryLevel, _currentTDProStatus.Master.Charging, _currentTDProStatus.SignalStrength);
            UpdateThimblesStatus(_currentTDProStatus);
            _touchDiverProStatusReceived = false;
        }
    }
    
    private void UpdateInfoPanel(string deviceMacAddress, int batteryLevel, bool charging, float signalStrenght)
    {
    ...
    private void UpdateCalibrationStatus()
    {
        ...
        {
            bool needCalibration = _deviceConnected && (DateTime.UtcNow - DateTime.Parse(_currentTDProStatus.SensorsCalibDate)).TotalDays > SensorsCalibDateDays;
            if(SensorsCalibDateImage.activeSelf != needCalibration) SensorsCalibDateImage.SetActive(needCalibration);

            if (_watchPanel != null)
                _watchPanel.SetData(_currentTDProStatus);

#if UNITY_ANDROID && !UNITY_EDITOR
        return;
#endif

            if (CalibrationText.text == "")
            {
                if(_calibrationPanelObj.activeSelf) _calibrationPanelObj.SetActive(false);
                if (!_signalPanelObj.activeSelf) _signalPanelObj.SetActive(true);
            }
            else
            {
                if (!_calibrationPanelObj.activeSelf) _calibrationPanelObj.SetActive(true);
                if (_signalPanelObj.activeSelf) _signalPanelObj.SetActive(false);
            }
        }
        else
        {
            ...
            {
                _watchPanel.SetG1();
                _watchPanel.SetBatteryG1(_currentTDStatus);
                SensorsCalibDateImage.SetActive(false);
            }
        }
    }
    ...
    {
        UpdateInfoPanel(MacAddressText.text, 0, false, 0);

        switch (WeArtController.Instance.DeviceGeneration)
        {
