...
namespace WeArt.TouchUI
{
    public class TouchDIVERInfo : MonoBehaviour
    {
        ...
        public HandSide handSide = HandSide.Unknown;
        ...
        private void Start()
        {
            // Change hand side button is available only for TD Devices
            if ( WeArtController.Instance.DeviceGeneration == DeviceGeneration.TD_Pro)
            {
                changeHandButton.interactable = false;
            }
            changeHandButton.onClick.AddListener(ChangeHandSide);
            handSideText.text = HandSide.Unknown.ToString();
