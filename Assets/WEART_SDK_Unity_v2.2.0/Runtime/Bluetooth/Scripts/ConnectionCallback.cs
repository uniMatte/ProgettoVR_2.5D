...
namespace WeArt.Bluetooth
{
    ...
    public class ConnectionCallback : AndroidJavaProxy
    {
        ...
        {
            WeArtLog.LogFile(Utils.LogEnums.LogLevel.VERBOSE, Utils.LogEnums.PackTag.EVENTS, $"PLUGIN CALLED OnConnected EVENT FOR DEVICE: {deviceMacAddress}");
            OnDeviceConnected?.Invoke(this.bluetoothManager.GetDeviceFromMac(deviceMacAddress));
        }
        ...
        {
            WeArtLog.LogFile(Utils.LogEnums.LogLevel.VERBOSE, Utils.LogEnums.PackTag.EVENTS, $"PLUGIN CALLED OnDeviceReady EVENT FOR DEVICE: {deviceMacAddress}");
            OnDeviceReady?.Invoke(this.bluetoothManager.GetDeviceFromMac(deviceMacAddress));
        }
        ...
        {
            WeArtLog.LogFile(Utils.LogEnums.LogLevel.VERBOSE, Utils.LogEnums.PackTag.EVENTS, $"PLUGIN CALLED OnDisconnected EVENT FOR DEVICE: {deviceMacAddress}");
            OnDeviceDisconnected?.Invoke(this.bluetoothManager.GetDeviceFromMac(deviceMacAddress));
        }
    }
